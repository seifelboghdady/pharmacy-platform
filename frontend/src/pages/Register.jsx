...
export default function Register() {
  ...
  const validate = () => {
    ...
    if (!form.pharmacy.trim()) {
      e.pharmacy = "Pharmacy name is required.";
    }
  ...
  const handleSubmit = async (ev) => {
    ...
    try {
      ...
      const response = await fetch(
        "https://pharmteck.up.railway.app/api/auth/register",
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            ...
            pharmacyName: form.pharmacy.trim(),
      ...
      if (!response.ok) {
        setErrors({
          server: data.message || "Unable to create account.",
        });
        return;
      }
    ...
    >
      <div
        ...
        style={{
          width: "40%",
          background:
            "linear-gradient(160deg, #00ACC1 0%, #1565C0 50%, #0D47A1 100%)",
          padding: "3rem",
          position: "relative",
          overflow: "hidden",
        }}
